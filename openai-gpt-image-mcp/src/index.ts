...
const loadEnvFile = (filePath: string) => {
  try {
    ...
    console.warn(`Loaded environment variables from ${filePath}`);
  } catch (error) {
...
if (envFileArgIndex !== -1 && cmdArgs[envFileArgIndex + 1]) {
  console.warn("Loading environment variables from file:", cmdArgs[envFileArgIndex + 1]);
  const envFilePath = cmdArgs[envFileArgIndex + 1];
  loadEnvFile(envFilePath);
} else {
  console.warn("No environment file provided");
}
